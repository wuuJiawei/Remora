...
- `RemoraTerminal`: ANSI parsing, screen buffer state, rendering, input mapping, and
...
4. `RemoraTerminal` parses terminal output, updates the screen buffer, and renders the
   terminal surface back into the app UI.

## Boundary Rules

- `RemoraApp` owns user-facing workflow and presentation state.
- `RemoraTerminal` stays focused on terminal emulation and rendering concerns.
