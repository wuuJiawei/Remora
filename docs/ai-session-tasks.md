...
- [x] `status: done` `id: AI-101` `[P0]` 新增 Sidecar 面板 UI（可收起/展开）；验收：可发送消息并显示 AI 回复占位。
...
- [x] `status: done` `id: AI-404` `[P0]` 增加 AI 全局开关（默认开启，可在 Settings 关闭）；验收：关闭后隐藏 sidecar/inline/smart assist 入口并阻断 provider 调用，运行时切换立即生效。
