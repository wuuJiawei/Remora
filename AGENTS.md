...
- 对于独立 macOS 工具窗口、三栏/双栏资源管理器、Finder 风格列表、侧边栏和表格等高频 UI，优先使用 AppKit 原生窗口栈（如 `NSToolbar`、`NSSplitViewController`、`NSOutlineView`、`NSTableView`），减少 SwiftUI 自绘布局和材质模拟；只有在简单嵌入或现有模块明确依赖 SwiftUI 时才使用 SwiftUI。
- 所有可点击按钮在点击时必须提供轻量微动画或等效的即时交互反馈（例如按压缩放、淡入淡出、状态过渡、进度反馈、飞行动画等）。动画应短促、可预测、低开销，不得阻塞主线程、影响连续点击响应或破坏原生 macOS 操作手感。
...
- Remora 本地问题排查时，优先直接查看本地文件日志，不要默认等待用户手动粘贴控制台输出。
- 主日志路径：`~/Library/Logs/Remora/app.log`
- 轮转日志路径：`~/Library/Logs/Remora/app.1.log`、`~/Library/Logs/Remora/app.2.log` 等
- 文件传输诊断日志路径：`~/Library/Logs/Remora/file-transfer-diagnostics.log`
- 当用户反馈“崩溃 / 卡住 / 某一步没反应 / 复现不了的偶发问题”时，先读上述日志，再决定是否追加更多埋点。
